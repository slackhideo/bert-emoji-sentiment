...
from transformers.trainer import SequentialDistributedSampler, get_tpu_sampler
...
#                SequentialSampler(train_dataset)
    ...
    def train(self, model_path: Optional[str] = None):
        ...
        # Keep tabs on the best checkpoint
        def _get_best_ckpt_info(path):
            if os.path.exists(path):
                with open(path, "r") as best_ckpt_info:
                    global_step, best_perf_global_step, best_perf = (best_ckpt_info.read().split("|"))
                    global_step = int(global_step)
                    best_perf_global_step = int(best_perf_global_step)
                    best_perf = float(best_perf)
            else:
                global_step = -1
                best_perf_global_step = -1
                best_perf = -1

            logger.info("  [Best ckpt info] step = %d | best value step = %d | best value = %f",
                        global_step,
                        best_perf_global_step,
                        best_perf)

            return global_step, best_perf_global_step, best_perf

        best_ckpt_info_file = os.path.join(self.args.output_dir, "best_ckpt_info.txt")
        output_eval_file    = os.path.join(self.args.output_dir, "eval_results.txt")

        # Load previous values from the last checkpoint
        global_step, best_perf_global_step, best_perf = _get_best_ckpt_info(best_ckpt_info_file)
            ...
            for step, inputs in enumerate(epoch_iterator):
                ...
                if (step + 1) % self.args.gradient_accumulation_steps == 0 or (
                    ...
                    if self.args.evaluate_during_training and self.global_step % self.args.eval_steps == 0:
                        ...
                        # Log the evaluation metrics to the output file
                        writer.write(("=" * 50) + "\n")
                        for key in sorted(metrics.keys()):
                            writer.write(f"{key} = {str(metrics[key])}\n")
                        writer.write(f"best = {best_perf} (step {best_perf_global_step})\n")
                        writer.flush()

                        # Save the checkpoint with the best result
                        if metrics["eval_acc"] > best_perf:
                            best_perf_global_step = self.global_step
                            best_perf = metrics["eval_acc"]
                            logger.info("New best result found! Step = %d, acc = %f", best_perf_global_step, best_perf)
                            metrics_improved = True

                        with open(best_ckpt_info_file, "w") as best_ckpt_info:
                            best_ckpt_info.write(f"{self.global_step}|{best_perf_global_step}|{best_perf}")
                    ...
                    if self.args.save_steps > 0 and self.global_step % self.args.save_steps == 0:
                        ...
                        # Only save the checkpoint if this iteration improved the result
                        if metrics_improved:
        ...
        # Preserve the best checkpoint
        src_ckpt_path = os.path.join(self.args.output_dir, f"{PREFIX_CHECKPOINT_DIR}-{best_perf_global_step}")
        dst_ckpt_path = os.path.join(self.args.output_dir, f"{PREFIX_CHECKPOINT_DIR}-best")
        if os.path.exists(src_ckpt_path):
            logger.info(f"Saving {src_ckpt_path} to {dst_ckpt_path}")
            shutil.move(src_ckpt_path, dst_ckpt_path)

        # Load the best checkpoint
        logger.info(f"Loading {dst_ckpt_path}")
        self.model = self.model.from_pretrained(dst_ckpt_path).to(self.args.device)
    ...
    def _training_step(
        ...
        # Classes weights for unbalanced datasets
        if self.classes_weights is not None:
            inputs["classes_weights"] = self.classes_weights
